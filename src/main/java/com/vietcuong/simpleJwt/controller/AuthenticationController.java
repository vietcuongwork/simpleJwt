...
import com.vietcuong.simpleJwt.common.ResponseStatus;
...
public class AuthenticationController {
    ...
    public AuthenticationController(AuthenticationService authenticationService, UserService userService, UserDetailsServiceImpl userDetailsService, TokenService tokenService) {
    ...
    public AuthenticationErrorResponse register(
            @RequestBody
            User userRequest) {
        AuthenticationErrorResponse response = new AuthenticationErrorResponse();
        Map<String, String> detail = new HashMap<>();
        if (userService.existsByUsername(userRequest.getUsername())) {
            response.setStatusCode(ResponseStatus.GlobalError.USER_REGISTRATION_ERROR.getCode());
            response.setDescription(ResponseStatus.GlobalError.USER_REGISTRATION_ERROR.getDescription());
            detail.put("error", "Username has already been used");
            response.setDetail(detail);
            return response;
        }
        response.setStatusCode(ResponseStatus.GlobalError.USER_REGISTRATION_SUCCESS.getCode());
        response.setDescription(ResponseStatus.GlobalError.USER_REGISTRATION_SUCCESS.getDescription());
...
/*    @PostMapping("/login")
    ...
    @PostMapping("/login")
    public AuthenticationErrorResponse login(
            @RequestBody
            User userRequest) {
        AuthenticationErrorResponse response = new AuthenticationErrorResponse();
        Map<String, String> detail = new HashMap<>();
